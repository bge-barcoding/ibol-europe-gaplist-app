...
lk_logger = logging.getLogger('load_naturalis')
...
def init_record_fields(row):
    ...
    if row['NSR Species']:
        ...
        if len(parts) > 1:
            ...
            record['taxon'] = row['NSR Species']
    else:
        lk_logger.warning("Record has no Taxon field, skipping...")
        return None

    # set up the other required fields
    record['kingdom'] = row['NSR Kingdom']
    record['marker'] = row['Marker']
    record['catalognum'] = row['Collection ID']
    record['institution_storing'] = 'Naturalis Biodiversity Center'  # same as in BOLD
    record['identification_provided_by'] = ""
    record['external_id'] = row['Consensus ID']
    record['locality'] = 'Netherlands'

    return record


def load_naturalis(input_file, encoding='utf-8'):
...
if __name__ == '__main__':
    ...
    session = Session()
    main_logger.info('Load Naturalis file "%s"', args.tsv)
    load_naturalis(args.tsv)
    session.commit()
